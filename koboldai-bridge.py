...
try:
    ...
    class temp(object):
        def __init__(self):
            ...
            self.kai_url = "http://localhost:5106"
            # Give a cool name to your instance
            self.kai_name = f"Random533256"
...
class kai_bridge():
    ...
    def validate_kai(self, kai):
        if self.model != '' and (self.last_retrieved is None or time.time() - self.last_retrieved <= 10):
            logger.info("Waiting...")
            return True
        ...
        while self.run:
            ...
            if current_id:
                ...
                if not pop:
                    ...
                    continue
                if not pop["id"]:
                    logger.info(f"Server {cluster} has no valid generations to do for us. Skipped Info: {pop['skipped']}.")
                    time.sleep(interval)
                    continue
                current_id = pop['id']
            ...
            logger.info(f"Job received from {cluster} for {current_payload.get('max_length',80)} tokens and {current_payload.get('max_context_length',1024)} max context. Starting generation...")
            request_serialized_response = json.dumps(pop)
            logger.info(f"Request is {request_serialized_response}")
            ...
            if gen_req.status_code == 503:
                logger.info(f'KAI instance {kai_url} Busy (attempt {loop_retry}). Will try again...')
                loop_retry += 1
                continue
            ...
            if return_error:
                ...
                try:
                    ...
                    logger.info(f"Request {request_serialized_response} returned: {json.dumps(req_json)}")
                except KeyError:
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument('-v', '--verbosity', action='count', default=3, help="The default logging level is ERROR or higher. This value increases the amount of logging seen in your screen")
    ...
    arg_parser.add_argument('--log_file', action='store_true', default=True, help="If specified will dump the log to the specified file")
    args = arg_parser.parse_args()
    set_logger_verbosity(args.verbosity)
    if args.log_file or True:
        logger.add("koboldai_bridge_log.log", level="INFO")
